...
instance.web.ListView.List = instance.web.Class.extend( /** @lends instance.web.ListView.List# */{
    ...
    init: function (group, opts) {
        ...
        this.record_callbacks = {
            'remove': function (event, record) {
                ...
                $row.remove();
                self.total_change(self.records)
            },
        ...
        //bind each events to the current template
    ...
    get_total:function(records){
    	var $total=0.00;
    	for(var idx=0;idx<records.length;idx++){
    		var $price_subtotal=records.at(idx).get('price_subtotal');
    		if ($price_subtotal){
    ...
    toMoney:function(amount,c,d,t){
    	Number.prototype.formatMoney = function (c, d, t){
            var n = this, 
             ...
             return  Number(amount).formatMoney(c,d,t);
    }
    ,
    total_change:function(records){
    	$amount_total=this.toMoney(this.get_total(records),2);
    	$target=$('span').filter('.oe_subtotal_footer_separator').children();
    	if($target.attr('class')!==undefined && $target.is("span")){
    		$target.text($amount_total);
    	}
    },
